...
\begin{figure}[h]
\begin{center}
\epsfig{width=0.76 in, file=logo.jpg}
\end{center}
\end{figure}
...
guidance. Special thanks to \textbf{Mr. Girish N Gopal} (Technical Advisor at Tech11 Software) for introducing us to the concept of Continuous Integration and for his guidance throughout the completion of the project.
...
\par
   For version control, Created a Repository named LogicBaker in Git-Hub and cloned to individual systems. Created modules given to each member in java After Coding the individual modules the team members commit to Git-Hub, irrespective of the platform which each one has build.
   
   \par
   The integration is much helpful via version controlling.  But to modify the existing code pushed by other individual member, it was time consuming. Because there was no standardization in the module codes. To standardize the project we mavenized the project, thus the project had a standard and the reusing libraries  occur only a single copy in the entire project provided an active Internet connection is required at the time of cloning and first build. \par
   The main advantages of using a Continuous Integration is deploying the currently developed modules in the private online space. Thus every member of the project can view the project and suggest the changes by notifying the developer, thus it is helpful to the developers. \par
   We use jenkins as the C.I handler, Jenkins is an open source automation server written in Java. It is a server-based system running in a servlet container such as Apache Tomcat. Jenkins provides a automation platform which each module can be integrated and build and error can be notified to entire personnel participating in the project development.
   \par For unit testing while developing, the Test automation is added in jenkins. The test Automation plugin is added to Jenkins depending upon the nature of the development process dealt with. Which involves programming framework, language etc. Since we use Java , we use ﬁnd-bugs and Check style in Jenkins. Also added Jacoco , For code coverage
 \par
  For much more flexibility the entire CI process can be handled by Heroku. Heroku is a cloud Platform-as-a-Service (PaaS) supporting several programming languages that is used as a web application deployment model. \par
  ...
  New Relic plugin is added to the Calc Application(Node.Js). Its an application performance manager. It tracks real time response time and throughput.
   \par
   Compared the various CI development process using the testing calculator application and found it is much better than the traditional development process. The findings are noted and submitted to  Tech11 softwares and found much decrease in their development time and cost.
    \newpage
...
\end{center}
\end{figure}
 \par AWS act as a remote infrastructure. Jenkins can be accessed using the public ip address of the instance.
\pagebreak
\newpage
\subsection{Jenkins}
\par Whenever a change has occurred in the GitHub the change will be automatically pushed into the ci server ie, Jenkins.It is then build and is ready to deploy
\begin{figure}[h]
\begin{center}
...
\par The system “Continuous integration pipeline implementation for tech11 software” is tested in such a way that almost all errors that may occur are found and corrected. The test process carried out in this system includes the following:\\
...
Run Applications                                                                    & Developer can continuously integrate and  ready-to deploy. \\ \hline
...
\end{tabular}
\end{table}
\par Test case preparation helps the user and the developer to find and fix the errors easily and in advance. Continuous integration pipeline implementation for tech11 software is well tested with the proper test cases and thus passed a better unit test. Elaborated test cases also prepared subjected the system for thorough testing. The test cases prepared are in the above format.
